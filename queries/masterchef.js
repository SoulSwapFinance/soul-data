...
module.exports = {
    ...
    async pool({block = undefined, timestamp = undefined, pool_id = undefined, pool_address = undefined} = {}) {
        if(!pool_id && !pool_address) { throw new Error("soul-data: Pool ID / Address undefined"); }
...
const info = {
    ...
    callback(results) {
        return ({
            ...
            soulPerBlock: results.soulPerBlock / 1e18,
...
const pools = {
    ...
    callback(results) {
        return results.map(({ id, pair, allocPoint, lastRewardBlock, accSoulPerShare, balance, userCount, slpBalance, slpAge, slpAgeRemoved, slpDeposited, slpWithdrawn, timestamp, block, updatedAt, entryUSD, exitUSD, soulHarvested, soulHarvestedUSD }) => ({
            id: Number(id),
            pair: pair,
            allocPoint: Number(allocPoint),
            lastRewardBlock: Number(lastRewardBlock),
            accSoulPerShare: BigInt(accSoulPerShare),
            ...
            soulHarvested: Number(soulHarvested),
            soulHarvestedUSD: Number(soulHarvestedUSD)
         }));
    }
};
...
const user = {
    properties: [
        'id',
        'address',
        'pool { id, pair, balance, accSoulPerShare, lastRewardBlock }',
        'amount',
        'rewardDebt',
        'entryUSD',
        'exitUSD',
    ...
    callback(results) {
        return results.map(entry => ({
            ...
            soulHarvested: Number(entry.soulHarvested),
            soulHarvestedUSD: Number(entry.soulHarvestedUSD),
        }));
    }
};
    ...
    callback24h(results, results24h) {
        return results.map(result => {
            ...
            return ({
                ...
                soulHarvestedChange: (result.soulHarvested / result24h.soulHarvested) * 100 - 100,
                soulHarvestedChangeCount: result.soulHarvested - result24h.soulHarvested,
            });
        });
    }
}
