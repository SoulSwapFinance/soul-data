const { getWeek, subYears, startOfMinute, getUnixTime } = require('date-fns');

const { TWENTY_FOUR_HOURS } = require('./../constants');
const { dayData, tokenHourData, tokenDayData, pairHourData, pairDayData } = require('./exchange');

module.exports = {
	async factory() {
		const data = await dayData();
		const weeklyData = [];
	...
	async tokenHourly({ token_address = undefined, startTime = undefined }) {
		...
		const ethPrices = await { minTimestamp: startTime };
